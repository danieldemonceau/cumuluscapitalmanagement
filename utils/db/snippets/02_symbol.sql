...
SELECT ss.name name, ss.full_name full_name, ss.description description, st.id symbol_type_id, s.id sector_id, e.id exchange_id
FROM (
    VALUES (
            'SYF'
            , 'Synchrony Financial'
...
Available on the NYSE, LyondellBasell is listed under the ticker LYB. In addition to its other activities, the company also develops and licenses technologies for chemical and polyolefin processes and manufactures and sells polyolefin catalysts. LyondellBasell also refines crude oil into gasoline, diesel, and jet fuel. In addition to being one of the largest plastics, chemicals, and refining companies in the world, LyondellBasell is a leading producer of oxyfuels and the largest global licensor of polyolefin technologies as of December 2021.'
        ...
        , (
            'MT.US'
            , 'ArcelorMittal ADR'
            , 'ArcelorMittal is a steel manufacturing corporation with headquarters in Luxembourg City. Established in 1976, the company operates across the globe, with steel manufacturing facilities in 18 countries and customers spread across 160 countries. The firm trades on the NYSE with the symbol MT.'
            , 'Stock'
            , 'Materials'
            , 'NYSE'
        )
) ss(name, full_name, description, symbol_type_name, sector_name, exchange_name)
