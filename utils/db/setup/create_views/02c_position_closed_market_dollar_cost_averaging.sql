DROP VIEW IF EXISTS position_closed_market_dollar_cost_averaging;

CREATE OR REPLACE VIEW position_closed_market_dollar_cost_averaging AS
SELECT
    p.id id,
    p.status "status",
    mt.direction "type",
...
FROM
    ...
    JOIN etf s ON s.id = mt.security_id
    JOIN (
...
COMMENT ON VIEW position_closed_market_dollar_cost_averaging IS E'@name position_closed_market_dollar_cost_averaging\n@omit update,delete\nThis is the documentation.';
