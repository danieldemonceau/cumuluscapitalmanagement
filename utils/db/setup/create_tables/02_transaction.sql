...
CONSTRAINT constraint_transaction_type_predefined NOT NULL
...
CREATE TABLE transaction_history (LIKE transaction);

CREATE INDEX IF NOT EXISTS idx_transaction_type ON transaction(type);
CREATE INDEX IF NOT EXISTS idx_transaction_description ON transaction(description);
CREATE INDEX IF NOT EXISTS idx_transaction_execution_execution_timestamp ON transaction(execution_timestamp);
CREATE INDEX IF NOT EXISTS idx_transaction_broker_id ON transaction(broker_id);
CREATE INDEX IF NOT EXISTS idx_transaction_amount ON transaction(amount);
CREATE INDEX IF NOT EXISTS idx_transaction_sys_period ON transaction(sys_period);
