  ...
  return (
    <>
      <nav className="h-full bg-white">
        <div className="flex flex-row h-20 justify-center items-center space-x-10 text-center">
          <div className="absolute left-0 ml-6">
            <a href="https://fluffyclouds.com.au" target={`_self`}>
              <Image src={CloudImage} alt={`Cloud Icon`}></Image>
            </a>
          </div>
          <div className="group text-xl text-center text-black flex flex-col h-full align-middle justify-center">
            <Link href="/">Home</Link>
          </div>
          <div
      ...
      <nav
        className={`absolute z-50 left-0 right-0 m-auto my-auto bg-gray-200 flex justify-center items-center text-black h-auto transition ease-in-out duration-1000 delay-150
        ${strategyMenusIsVisible ? "nav-menu-active" : "nav-menu-inactive"}
        `}
        onMouseEnter={() => setStrategyMenusIsVisible(() => true)}
        onMouseLeave={() => setStrategyMenusIsVisible(() => false)}
      >
